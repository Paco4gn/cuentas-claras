...
test('balance list can hide and restore zero-balance people', async ({ page }) => {
  ...
  const reminderLink = await personSheet.getByRole('link', { name: /Recordar/i }).getAttribute('href')
  expect(decodeURIComponent(reminderLink ?? '')).toContain('queda pendiente')
  await personSheet.getByLabel('Cerrar ficha').click()
